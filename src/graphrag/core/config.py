...
def resolve_env_vars(config: any) -> any:
    """Recursively replace ${VAR} or $VAR in dict/list/str with environment variable values."""
    import re
...
from src.graphrag.core.embeddings.tigergraph_embedding_store import TigerGraphEmbeddingStore
from src.graphrag.core.llm_services import (
...
from src.graphrag.core.session import SessionHandler
from src.graphrag.core.status import StatusManager
...
if SERVER_CONFIG[-5:] != ".json":
    ...
    with open(SERVER_CONFIG, "r") as f:
        server_config = resolve_env_vars(json.load(f))
...
# direct importers (``from src.graphrag.core.config import embedding_store``).
...
def _build_embedding_store(graphname: str = "") -> TigerGraphEmbeddingStore:
    ...
    is_cloud = "tgcloud.io" in host.lower()
    # Resolve graphname: prioritize explicit arg, then db_config keys (allow graph_name alias)
    target_graph = graphname or db_config.get("graphname") or db_config.get("graph_name", "")
    ...
    conn = TigerGraphConnection(
        ...
        tgCloud=is_cloud,
        useCert=is_cloud or host.startswith("https://")
    )
