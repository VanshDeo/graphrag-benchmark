...
from src.graphrag.core.storage.azure_blob_store import AzureBlobStore
from src.graphrag.core.storage.google_blob_store import GoogleBlobStore
from src.graphrag.core.storage.s3_blob_store import S3BlobStore
from src.graphrag.core.py_schemas import BatchDocumentIngest, Document, DocumentChunk, KnowledgeGraph
from typing import List, Union
import json
from datetime import datetime
from src.graphrag.core.status import Status, IngestionProgress
from src.graphrag.core.extractors import LLMEntityRelationshipExtractor
...
class BaseIngestion:
    ...
    def chunk_document(self, document, chunker, chunker_params):
        if chunker.lower() == "regex":
            from src.graphrag.core.chunkers.regex_chunker import RegexChunker

            chunker = RegexChunker(chunker_params["pattern"])
        elif chunker.lower() == "characters":
            from src.graphrag.core.chunkers.character_chunker import CharacterChunker

            chunker = CharacterChunker(
                chunker_params.get("chunk_size", 0), chunker_params.get("overlap_size", -1)
            )
        elif chunker.lower() == "semantic":
            from src.graphrag.core.chunkers.semantic_chunker import SemanticChunker
            ...
            from src.graphrag.core.chunkers.html_chunker import HTMLChunker
            ...
            from src.graphrag.core.chunkers.markdown_chunker import MarkdownChunker
        ...
        chunks = [
            DocumentChunk(
                document_chunk_id=f"{document.document_id}_chunk_{i}", text=chunk
            )
