from src.graphrag.core.embeddings.embedding_services import EmbeddingModel
from src.graphrag.core.embeddings.base_embedding_store import EmbeddingStore
from src.graphrag.core.metrics.tg_proxy import TigerGraphConnectionProxy
from src.graphrag.core.llm_services.base_llm import LLM_Model
from src.graphrag.core.py_schemas import CandidateScore, CandidateGenerator, GraphRAGAnswerOutput, CommunityAnswer
from src.graphrag.core.utils.token_calculator import get_token_calculator
from src.graphrag.core.config import get_chat_config, get_embedding_store, get_graphrag_config
...
class BaseRetriever:
    ...
    def _install_query(self, query_name):
        ...
        with open(f"src/core/gsql/supportai/retrievers/{query_name}.gsql", "r") as f:
            query = f.read()
        res = self.conn.gsql(
            "USE GRAPH "
            + self.conn.graphname
